...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features = 36, hidden_size = 200, n_classes = 3, dropout_prob = 0.5):
        ...
        d = self.embed_size
        h = hidden_size
        self.embed_to_hidden_weight = nn.Parameter(torch.randn((d*m, h), dtype = torch.float32))
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        # b_1
        self.embed_to_hidden_bias = nn.Parameter(torch.randn(h, dtype = torch.float32))
        nn.init.uniform_(self.embed_to_hidden_bias)
        
        self.dropout = nn.Dropout(p = dropout_prob)
        
        # U
        self.hidden_to_logits_weight = nn.Parameter(torch.randn((h, n_classes), dtype = torch.float32))
        nn.init.xavier_uniform_(self.hidden_to_logits_weight)
        # b_2
        self.hidden_to_logits_bias = nn.Parameter(torch.randn(n, dtype = torch.float32))
        nn.init.uniform_(self.embed_to_hidden_bias)
    ...
    def forward(self, w):
        ...
        relu = torch.nn.ReLU()
        h = relu(embed @ self.embed_to_hidden_weight + self.embed_to_hidden_bias)
        h_dropout = self.dropout(h)
        
        logits = h_dropout @ self.hidden_to_logits_weight + self.hidden_to_logits_bias
