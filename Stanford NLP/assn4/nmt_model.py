...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # self.decoder =                      torch.nn.LSTM(input_size = embed_size + hidden_size, hidden_size = self.hidden_size, bias = True, bidirectional = False)
        ...
        # self.combined_output_projection =   torch.nn.Linear(self.hidden_size * 3, self.hidden_size)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # packed_X = torch.nn.pack_padded_sequence(X, source_lengths)
        # enc_hiddens = self.encoder(packed_X)
        # seq_unpacked, lens_unpacked = torch.nn.pad_packed_sequence(enc_hiddens, batch_first = True)
        
        
        # source_padded (src_len, b) -> [[Encoding layer]] (src_len, b, 2h)
        X = self.model_embeddings.source(source_padded)

        # packed_seq (src_len, b, 2h)
