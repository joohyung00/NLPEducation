...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = (outsideVectors.T) @ delta_y_hat
    gradOutsideVecs = delta_y_hat.reshape(-1, 1) @ centerWordVec.reshape(1, -1)
...
def negSamplingLossAndGradient(
    ...
    sigmoid_uo_vc = sigmoid(u_o @ v_c)
    loss = -np.log(sigmoid_uo_vc)
    gradCenterVec = (sigmoid_uo_vc - 1) * u_o
    
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = -(1 - sigmoid_uo_vc) * v_c
    
    for word_idx in negSampleWordIndices:
        u_w = outsideVectors[word_idx]
        sigmoid_uw_vc = sigmoid(-u_w @ v_c)
        
        loss += -np.log( sigmoid_uw_vc )
        gradCenterVec += (1 - sigmoid_uw_vc) * u_w
        gradOutsideVecs[word_idx] += (1 - sigmoid_uw_vc) * v_c
